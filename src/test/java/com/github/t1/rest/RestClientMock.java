...
public class RestClientMock extends ExternalResource {
    ...
    public static class RequestMock {
        ...
        public void reply(final String string) {
            when(request.execute(any(ResponseConverter.class))).then(new Answer<EntityResponse<Object>>() {
                @Override
                public EntityResponse<Object> answer(InvocationOnMock invocation) {
                    ...
                    return new EntityResponse<>(responseConverter, headers, inputStream);
                }
            });
        }
    }
    ...
    public RequestMock on(URI uri) {
        ...
        when(requestFactoryMock.createGetRequest(eq(uri), any(Headers.class))).thenReturn(getRequest);
